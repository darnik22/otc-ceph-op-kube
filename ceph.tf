...
resource "null_resource" "ceph-node-connect" {
  count = "${var.ceph-node_count}"
  # depends_on = ["null_resource.bastion", "openstack_networking_floatingip_v2.ceph-nodes"]
  connection {
    bastion_host = "${openstack_networking_floatingip_v2.oneprovider.address}"
...
resource "null_resource" "provision-ceph" {
  depends_on = ["null_resource.ceph-node-connect","openstack_compute_volume_attach_v2.vas",]
  # provisioner "local-exec" {
  #   command = "./local-setup.sh ${var.project} ${var.ceph-mon_count} ${var.ceph-node_count} ${var.client_count} ${var.provider_count} ${var.dnszone}"
  # }
  # provisioner "local-exec" {
